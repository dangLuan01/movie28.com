                            ...
                            <div class="col-12 col-sm-6 col-md-12">
                                <div class="form__img">
                                    ...
                                    <input id="form__img-upload" name="image" type="file"
                                        accept=".png, .jpg, .jpeg" />
                                         <img id="form__img" src="https://wsrv.nl/?url={{$params['item']['image']}}" alt="" />
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="form__group">
                                    <label>Name</label>
                                    <input type="text" class="form__input slug" name="name" id="name" placeholder="Collection Name" value="{{$params['item']['name']}}" />
                                </div>
                            </div>   
                            <div class="col-12">
                                <div class="form__group">
                            ...
                            <div class="col-12 col-lg-6">
                                <div class="form__group">
                                    ...
                                    <select class="js-example-basic-multiple select2" id="movie" name="movie_id[]" multiple="multiple">
                                        ...
                                        <option value="{{$movie['id']}}" {{ $params['item']['movies']->contains($movie['id']) == true ? 'selected' : '' }}>{{$movie['name']}}</option>    
                                        @endforeach
                                    </select>
                                </div>
                            </div> 
...
<script>
    $(document).ready(function() {
        $('#admin-{{ $params['prefix'] }}-form').submit(function(e) {
            ...
            $.ajax({
                type: 'POST',
                url: "{{ route($params['prefix'] . '.' . $params['controller'] . '.update', ['collection' => $params['id']]) }}",
                data: formData,
                headers: {
